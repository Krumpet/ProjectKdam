...
from Consts.SemesterCodes import SEMESTER
...
def enumerate_faculties():
    for i in range(100):
        try:
            # print("getting faculty " + str(i))
            get_courses_by(FAC=i)
        except:
            # print("error faculty " + str(i))
            pass
        else:
            yield str(i).zfill(2)
...
def fetch_all_courses():
    # print("a")
    with open(FILES.COURSE_LIST, 'w', encoding='utf8') as out, open('metadata/failed.txt', 'a') as failed:
        # print("b")
        numbers = COURSE_IDS
        out.write('{\n')
